    ...
    <>
      ...
      <main id="top">
        ...
        {/* Возможности — то, что реально отличает ролик-поздравление от
            ...
            Обещано здесь только то, что работает у ЛЮБОГО человека на
            любом режиме: кадр до генерации, до четырёх сцен, своя
            музыка файлом или ссылкой, титры. Голос отправителя назван
            вместе с режимом, в котором он доступен (Standard), — иначе
            это было бы обещание, упирающееся в отказ.

            Чего здесь намеренно НЕТ: наклеек и поиска музыки по
            свободным библиотекам. Обе фичи в продукте есть, но
            включаются ключами стенда (PIXABAY_API_KEY, JAMENDO/
            FREESOUND/MUBERT), и пока ключи не заданы, страница
            обещала бы то, чего человек не увидит. Появятся ключи —
            добавятся карточки.

            Иконки свои (`greet-feature-1…5`), а не `feature-N` с
            главной: те нарисованы под рекламный конвейер и молча дали
            бы поздравлению чужие картинки — та же находка 1.4, из-за
            которой у шагов свой набор. */}
