...
describe('обычный круг', () => {
  ...
  it('sessionId не виден в самой квитанции', () => {
    ...
    //
    // Идентификаторы здесь длиннее, чем в остальных тестах, и это не
    // косметика. Квитанция — 98 символов base64, и проверка «не
    // содержит подстроку» на КОРОТКОЙ строке ловит не утечку, а
    // случайное совпадение: двухсимвольный `d1` выпадал в шифротексте
    // сам по себе в 2,3 % случаев (замерено перебором на 20 000
    // квитанциях), а вызовов в тесте два — около 4,5 % падений на
    // ровном месте. Набор зеленел через раз, и виноват был тест, а не
    // код. На идентификаторах такой длины вероятность совпадения
    // исчезающая, а смысл проверки тот же.
    const ids = { sessionId: 'relay-session-8f3ac1', draftId: 'draft-2b7e9d' };
    expect(issue(ids)).not.toContain(ids.sessionId);
    expect(issue(ids)).not.toContain(ids.draftId);
  });
