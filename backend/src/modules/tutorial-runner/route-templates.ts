 ...
 * фронтенда эту таблицу нужно поправить руками. Настоящий cross-package
 * импорт стоил бы дороже, чем изредка поправить одну таблицу.
 *
 * ## Что этот приём УЖЕ стоил — и почему таблица теперь полная
 *
 * «Изредка поправить» не случилось ни разу. К 26.09.2026 во фронтенде
 * было 26 имён маршрутов, здесь — 21: отстали `site-tutorial`,
 * `greeting-video`, `testing`, `api-keys` и `invite`, добавленные после
 * этапа 97. Хуже самого пропуска было сообщение об ошибке: резолвер
 * отвечал «нет такого в `frontend/src/lib/router.ts`» — и это была
 * неправда, маршрут там был. Оператор по такой подсказке идёт искать
 * опечатку в сценарии, а не устаревшую копию. Формулировка исправлена
 * (см. `resolveScenarioRoute` ниже): теперь она называет обе возможные
 * причины и указывает на этот файл первым.
 *
 * Таблица приведена в соответствие этапом F ТЗ
 * `docs-tz/TZ-Enterprise-Tutorial-Landing.md`: там она понадобилась,
 * чтобы снять настоящие кадры мастера обучалки для лендинга.
 *
...
export interface FixtureRouteContext {
  /** Проект РЕКЛАМНОГО пайплайна (`SINGLE`/`LINE`) — тот, у которого
   *  есть товары. Не подставлять сюда проект другого типа: маршруты
   *  `project`/`item` строят путь к товару внутри него. */
  projectId?: string;
  itemId?: string;
  manifestId?: string;
  sessionId?: string;
  ...
  /** Проект-поздравление (`GREETING_VIDEO`). Фикстура его сегодня НЕ
   *  заводит — маршрут резолвится только если оператор создал такой
   *  проект руками, и поэтому в `ROUTE_DESCRIPTIONS` не предлагается. */
...
export const ROUTE_DESCRIPTIONS: Record<string, string> = {
  ...
  'site-tutorial':
    'мастер обучающего видео по сайту заказчика: вставить адрес → сервер открывает страницу и присылает кадр со списком полей и кнопок → заполнить и нажать → следующий кадр. Три стадии (`url`/`page`/`review`) живут на ОДНОМ этом маршруте и адрес не меняют, кроме необязательного последнего сегмента шага',
  plan: 'тариф и лимиты',
  channels: 'подключённые каналы публикации (YouTube/TikTok)',
  credits: 'баланс и история списаний',
  feed: 'импорт товаров фидом',
  'api-keys': 'ключи внешнего API (Premium) — выпуск, отзыв, вебхуки',
  invite: 'экран «Пригласить»: сколько генераций осталось и как получить ещё',
};
...
const ROUTE_BUILDERS: Record<string, RouteBuilder> = {
  ...
  // Последний сегмент (шаг визарда) НЕ добавляется: он необязателен по
  // построению (`frontend/src/lib/router.ts` — ссылки, выданные до
  // «Тонкой красной линии», обязаны продолжать работать), а стадию
  // визард всё равно выводит из того, что есть у черновика. Сценарию,
  // которому нужна конкретная стадия, честнее дойти до неё действиями.
  'site-tutorial': (ctx) =>
    ctx.clientSiteProjectId
      ? {
          ok: true,
          path: `/projects/${ctx.clientSiteProjectId}/site-tutorial`,
        }
      : missingFixtureData('clientSiteProjectId'),
  'greeting-video': (ctx) =>
    ctx.greetingProjectId
      ? { ok: true, path: `/projects/${ctx.greetingProjectId}/greeting-video` }
      : missingFixtureData('greetingProjectId'),
  'api-keys': () => ({ ok: true, path: '/api-keys' }),
  invite: () => ({ ok: true, path: '/invite' }),
  // Резолвится технически (маршрут без параметров), но сценариям не
  // предлагается и здесь отказывает явно: экран брифа тестировщика
  // открыт только тем, у кого активирован доступ, а фикстурный
  // пользователь тестировщиком не является — обход снял бы экран отказа
  // и записал бы его как «экран продукта».
  testing: () =>
    unsupported(
      'экран брифа тестировщика: фикстурный пользователь не тестировщик, и маршрут ответит отказом в доступе, а не содержимым',
    ),
...
export function resolveScenarioRoute(
  ...
  if (!builder) {
    return {
      ok: false,
      // Прежняя формулировка утверждала «нет такого в
      // frontend/src/lib/router.ts» — и в половине случаев это была
      // неправда: маршрут там был, отстала ЭТА копия таблицы (см.
      // доккомментарий файла). Оператор шёл искать опечатку в сценарии
      // вместо того, чтобы дописать строку сюда.
      reason: `имя маршрута «${routeName}» не найдено в таблице route-templates.ts. Либо это выдумка генератора, либо маршрут во frontend/src/lib/router.ts появился позже, а копию здесь не дописали — проверьте оба места`,
    };
  }
  return builder(ctx);
}
