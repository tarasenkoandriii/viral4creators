...
export interface SynthesisResult {
  ...
  characters: number;
  /**
   * Длительность синтезированной дорожки в секундах — измеренная по
   * самим байтам (`common/mp3-duration.ts`), а не оценённая по числу
   * символов (этап 138, §5 ТЗ TZ-Multilingual-YouTube.md).
   *
   * Шапка этого файла обещала «текст → mp3 плюс длительность» с самого
   * начала, но поля не было: провайдеры её не отдают, а расход считается
   * по символам, и никому не мешало. Мешать начинает на альтернативных
   * звуковых дорожках: YouTube требует, чтобы дорожка была примерно той
   * же длины, что ролик, а перевод почти никогда не совпадает по длине
   * с оригиналом — проверить это нечем, пока длительность неизвестна.
   *
   * `null` — «измерить не удалось» (нераспознанный файл), а не «ноль
   * секунд»: путать эти два значения нельзя, на нуле любая проверка
   * длины решит, что дорожка не годится.
   */
