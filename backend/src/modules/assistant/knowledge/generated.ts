...
export const ASSISTANT_KNOWLEDGE_BUILT_AT = '2026-09-15T03:59:50.185Z';
...
export const ASSISTANT_STEPS: Record<string, AssistantStepItem[]> = {
  ru: [
    ...
    {
      title: 'Проверьте и заберите',
      text: 'Отдельная проверка на артефакты предложит исправленный промпт для повторной генерации. Готовый ролик — прямой ссылкой на файл; можно поставить его в очередь на модерацию для публикации.',
      details: [
        'Проверка на артефакты и саундчек (Standard+)',
        'Публикация и страница для шеринга (Standard+, нужен вход)',
        'Экспорт под несколько площадок сразу — без повторной оплаты рендера (Standard+)',
  ...
  uk: [
    ...
    {
      title: 'Перевірте і заберіть',
      text: 'Окрема перевірка на артефакти запропонує виправлений промпт для повторної генерації. Готовий ролик — прямим посиланням на файл; можна поставити його в чергу на модерацію для публікації.',
      details: [
        'Перевірка на артефакти та саундчек (Standard+)',
        'Публікація та сторінка для шерингу (Standard+, потрібен вхід)',
        'Експорт під кілька площадок одразу — без повторної оплати рендеру (Standard+)',
  ...
  en: [
    ...
    {
      title: 'Review and grab it',
      text: 'A separate artifact check will propose a fixed prompt for a re-generation. The finished video is a direct file link; you can also queue it for moderation to publish.',
      details: [
        'Artifact check and sound check (Standard+)',
        'Publication and a shareable page (Standard+, sign-in required)',
        'Export to several platforms at once — no extra render charge (Standard+)',
  ...
  de: [
    ...
    {
      title: 'Prüfen und abholen',
      text: 'Eine separate Artefaktprüfung schlägt einen korrigierten Prompt für eine erneute Generierung vor. Das fertige Video steht als direkter Datei-Link zur Verfügung; es lässt sich auch zur Moderation für die Veröffentlichung einreichen.',
      details: [
        'Artefaktprüfung und Sound-Check (Standard+)',
        'Veröffentlichung und Seite zum Teilen (Standard+, Anmeldung nötig)',
        'Export für mehrere Plattformen zugleich — ohne erneute Render-Gebühr (Standard+)',
  ...
  es: [
    ...
    {
      title: 'Revisa y descárgalo',
      text: 'Una verificación aparte de artefactos propondrá un prompt corregido para una nueva generación. El video terminado se entrega como un enlace directo al archivo; también puedes ponerlo en la cola de moderación para publicarlo.',
      details: [
        'Verificación de artefactos y de audio (Standard+)',
        'Publicación y página para compartir (Standard+, requiere inicio de sesión)',
        'Exportación a varias plataformas a la vez — sin pagar el renderizado otra vez (Standard+)',
