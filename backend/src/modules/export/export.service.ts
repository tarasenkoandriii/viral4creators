...
export class ExportService {
  ...
  async startRerender(
    ...
    try {
      ...
      //
      // Найдено при аудите (ТЗ §10–11): без явного provider/resolution
      // экспорт исходного Grok-ролика в другой формат тихо уезжал бы на
      // Veo — тот же класс ошибки, что уже был в admin-retry.
      //
      // Найдено при ПОВТОРНОМ аудите (ТЗ §9, этап 4 плана §14): та же
      // логика касается длины цепочки — без неё переэкспорт
      // 56-секундного ролика в другой формат тихо давал бы обычные 8
      // секунд вместо той же длины в новом формате.
