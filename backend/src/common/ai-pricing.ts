...
export type AiOperation =
  ...
  | 'analysis-translate'
  // Перевод заголовка и описания ролика на локали продукта перед
  // загрузкой на площадку (этап 137) — отдельно от 'analysis-translate':
  // тот переводит разбор ДЛЯ ПОЛЬЗОВАТЕЛЯ на экране, этот — метаданные,
  // которые уезжают наружу, и в отчёте видно, сколько стоит
  // мультиязычность канала.
  | 'publication-translate'
  // Перевод реплики под хронометраж для альтернативной звуковой
  // дорожки (этап 138) — отдельно от 'publication-translate': тот
  // переводит метаданные, этот — то, что будет ЗВУЧАТЬ, и у него свой
  // повтор «на N % короче», за который тоже платят.
  | 'track-translate'
...
export const AI_OPERATION_LABEL: Record<AiOperation, string> = {
  ...
  'publication-translate': 'Перевод описания для площадки',
  'track-translate': 'Перевод реплики под хронометраж',
