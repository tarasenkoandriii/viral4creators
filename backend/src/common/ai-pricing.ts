...
export const MODEL_RATES: Readonly<Record<string, ModelRate>> = {
  ...
  // плана §14) — ради `negativePrompt`, которого нет на 3.1. Точный
  // ID модели и цена — НЕ подтверждены официально (Veo 3.0 к тому же
  // deprecated, §9.2 того же ТЗ) — ПРОВЕРИТЬ перед стройкой строже
  // обычного для этого файла.
  'veo-3.0-generate-001': {
    provider: 'VEO',
    perSecond: 0.4 * USD,
    note: 'ID модели и цена — не подтверждены официально, Veo 3.0 deprecated — ПРОВЕРИТЬ обязательно перед первым реальным вызовом',
  },
