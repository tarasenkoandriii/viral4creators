...
export interface GeneratedVideo {
  ...
  /** ID задачи Hedra, для `provider === 'hedra'` — по той же причине,
   * по которой `grokRequestId` отделён от `veoOperationName`: разные
   * провайдеры, разный формат ID. */
  hedraJobId?: string;

  /**
   * Речь УЖЕ внутри файла, синтезировать её второй раз нельзя.
   *
   * У аватар-ролика озвучка — это ВХОД: мы синтезируем реплику сами и
   * отдаём её Hedra вместе с портретом. Постобработка по умолчанию
   * читает отсутствие снимка бренда как режим `voiceover` и кладёт свою
   * дорожку поверх — здесь это дало бы две одинаковые речи со сдвигом,
   * ровно тот брак, из-за которого у ветки Grok появился
   * `silentSource`. Флаг закрывает вопрос на уровне данных, а не
   * договорённости между двумя сервисами.
   *
   * Не то же самое, что `silentSource`: тот говорит «у файла НЕТ
   * звуковой дорожки», этот — «дорожка есть, и она уже наша».
   */
  speechBakedIn?: boolean;
