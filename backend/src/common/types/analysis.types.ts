...
export interface VideoAnalysis {
  ...
  /**
   * Доп. запрос владельца продукта — короткий образец РЕЧИ (не текста на
   * экране, см. `OnScreenTextMoment` в prompt.types.ts) из оригинального
   * референсного видео, вычлененный отдельным дешёвым вызовом из
   * `sceneBreakdown` (`AnalysisService.extractOriginalDialogueSample()`).
   * Нужен для пробы голоса — дать пользователю послушать, как КАНДИДАТ-
   * ГОЛОС (из каталога ElevenLabs/Resemble) прочтёт ТЕ ЖЕ реплики, что
   * были в оригинале — НЕ клонирование голоса диктора оригинала (это
   * был бы чужой голос без его согласия, тот же принцип, что уже
   * защищает форма клонирования явным чекбоксом согласия).
   * `undefined` — ещё не вычленён; `null` — вычленён, но реплик не было
   * (значит превью с этим текстом недоступно, не значит «повторить»).
   */
  originalDialogueSample?: string | null;
