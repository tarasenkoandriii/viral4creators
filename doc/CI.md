...
прогонялись руками на каждом этапе: тогда 442 теста (сейчас 5008), 17
написанных вручную (сейчас 107)
...
| `frontend` | `tsc`, **`typecheck:scripts`** (типы самих проверочных скриптов), eslint через `npm run lint` (с `--report-unused-disable-directives`), 41 unit-скрипт `npx tsx frontend/scripts/*.test.ts`, `vite build` |
