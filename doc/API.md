...
| `GET/POST /api/admin/tester-invites` | оператор | приглашения тестировщиков (этап 155): список с готовыми ссылками `t.me/<bot>?start=t_<токен>` и создание. Одноразовое, привязывается к первому активировавшему. С этапа 159 приглашение несёт ещё две вещи: `freeOutsideProject` (клон голоса, озвучка, скетч, поиск — они не принадлежат проекту, и раньше открывались неявно, «если отмечены все три сценария») и `dailyLimitUsd` (свой суточный потолок; общий — это потолок НА КАЖДОГО тестировщика). Оба переносятся на пользователя при активации. Там же `brief` — что проверять: текст для экрана `#/testing`, свой у каждого тестировщика, потому что общий на всех означал бы, что каждый читает инструкцию для кого-то другого |
...
| `GET /api/me/test-tickets/brief` | тестировщик | экран `#/testing` (§2.3): ЧТО ПРОВЕРЯТЬ (текст брифа из приглашения), что открыто и до какого числа, сколько потрачено из потолка, свои находки со статусами и `ticketsTotal` — список обрезан, и подпись обязана знать полное число. Он же — ответ на «нужен ли лендинг»: нет, потому что здесь человек уже аутентифицирован и страницу нельзя переслать. Текста ответов оператора здесь НЕТ: ответ приходит в личку, и второе его место разошлось бы с первым |
...
| `PATCH /api/admin/settings/audio-separation` | оператор | включить/выключить сохранение фона (`state`: `on`/`off`) — третий уровень отката: «звучит плохо» автоматика не распознаёт, решение принимает человек и без деплоя |
...
| `POST /api/admin/tutorial-runner/seed-fixture-user` | оператор | заводит/обновляет фикстурного пользователя (§3.3 doc/TMA-UI-SNAPSHOT-AND-TUTORIAL-VIDEO-SPEC.md, этап 97) — та же идемпотентная цепочка upsert'ов (пользователь → манифест бренда → персонаж → проект → товар → сессия с готовым роликом, с этапа G ТЗ docs-tz/TZ-Enterprise-Tutorial-Landing.md ещё и проект третьего типа `CLIENT_SITE` для мастера обучалки — без черновика, кадры подделывать нечем), что раньше требовала ручного CLI-запуска `scripts/seed-fixture-user.ts` с прод `DATABASE_URL` (этап 105); `telegramId` берётся из `FIXTURE_TELEGRAM_ID` окружения, не из тела запроса — 400, если переменная не задана |
| `POST /api/admin/ui-snapshot/run` | оператор | разовый прогон UI-снимков по требованию (этап H ТЗ docs-tz/TZ-Enterprise-Tutorial-Landing.md): тело `{locale?, theme?, routeKeys?, unmasked?}`, умолчания те же, что у крона (`ru`, `light`, пять MVP-маршрутов, с масками). `unmasked: true` снимает БЕЗ маскирования `[data-qa-mask]` — такой снимок не пишет строку `UiSnapshot`, не сравнивается и не шлёт тревогу «изменилось» (иначе подменил бы базовый отпечаток и крон закричал бы на собственную картинку), файл ложится под префикс `qa-shots/` вместо `qa-snapshots/`, адрес возвращается в `outcomes[].blobUrl`. Локаль и тема с этого этапа не просто записываются в строку, а ВЫСТАВЛЯЮТСЯ в браузере через те же ключи `localStorage`, что пишет продукт (`v4c_locale`/`v4c_theme`) — до этого поля были ярлыками, совпадавшими с реальностью случайно |
