...
| `GET/POST /api/admin/tester-invites` | оператор | приглашения тестировщиков (этап 155): список с готовыми ссылками `t.me/<bot>?start=t_<токен>` и создание. Одноразовое, привязывается к первому активировавшему |
| `POST /api/admin/tester-invites/:id/revoke` | оператор | отзыв с причиной. Гасит и уже выданный доступ: иначе «отозвал» означало бы только «ссылка не работает», а человек продолжал бы тратить |
...
| `GET /api/admin/test-tickets/progress` | оператор | покрытие сценариев и сводка по каждому тестировщику. Считается из УЖЕ существующих данных (сессия → проект → тип → сценарий) агрегатом в БД, без единой новой таблицы. Истёкший `testAccessUntil` закрывает сценарии независимо от галочек: сценарий, по которому тишина, виден сразу |
