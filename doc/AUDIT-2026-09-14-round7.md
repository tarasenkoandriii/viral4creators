...
### М-5.6 (средняя, ИСПРАВЛЕНО 15.09.2026 по замечанию владельца «озвучка снова veo»). Умолчание `voiceover` не доходило до записей без поля
`normalizeVoiceMode(undefined)` → `LEGACY 'veo'`; плюс `?? 'veo'` в
форме манифеста (`ManifestScreen.tsx` — новый манифест сохранял голос
модели ЯВНО, обходя DEFAULT колонки и миграцию), в переключателе мастера
и в `useWorkflow.changeVoiceMode`. **Сделано:** одно умолчание на всю
систему — `normalizeVoiceMode` откатывается на `DEFAULT_VOICE_MODE`
(`voiceover`), фронтовые `?? 'veo'` → `?? 'voiceover'`; спеки
`postprod`/`snapshot` обновлены. Следствие: старый снимок без поля при
повторном рендере идёт со своим голосом (новый платный рендер по
текущему правилу). Сессия без манифеста по-прежнему без переключателя —
режим у неё теперь `voiceover`; переключатель на уровне сессии остаётся
отдельной задачей. Бренды, уже сохранённые формой с `veo`, миграция
задним числом не переведёт — один `UPDATE` (см. ответ владельцу).
...
- Живые контракты xAI: имена oneof-ключей REST-пачки. Первая живая
  пачка (14.09.2026, 23:33) вернула 422 со списком допустимых вариантов —
  ключи исправлены на `video_generation`/`video_extension` (были выведены
  по аналогии с `chat_get_completion` как `video_generate_video`).
  Вторая живая пачка (23:41) — базовый сегмент через `video_generation`
  прошёл (форма тела подтверждена). Третья (23:42, расширение) — ошибка
  xAI «Video extension is not supported for this model» для
  `grok-imagine-video-1.5`: расширение вынесено на отдельную модель
  `config.grok.videoExtendModel` (`GROK_VIDEO_EXTEND_MODEL`, по умолчанию
  `grok-imagine-video`, как в примере docs.x.ai) — и в синхронном
  `/v1/videos/extensions`, и в батче; ставки `grok-imagine-video:480p/720p`
  добавлены как у `-1.5` (отдельной строки в прайсе не найдено —
  проверить по списанию). Переопределение ключей через
  `GROK_BATCH_VIDEO_REQUEST_KEY`/`…_EXTEND_KEY` оставлено.
