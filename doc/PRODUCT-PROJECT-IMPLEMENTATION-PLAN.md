...
- Backend: **219 маршрутов в 46 контроллерах** (214/44 после этапа 82 —
  ...
  валидна); все **108** миграций подряд на чистом Postgres 16
  (**78** таблиц — этап 122 добавил колонку `liveData` на `sessions`,
  ...
  **5101 тестов / 324 набора** (2051/145 после аудита кода трёх
  ...
  Frontend: tsc, eslint 0, vite build, **42** unit-скрипта
...
- **Проверено.** 1022 теста / 87 наборов (+9), пороги покрытия держатся,
...
- **Расшифровка сохраняется сразу** (В-1.7): `updateItem` в момент
  ...
  «Сохранить».
- **Проверено.** 1030 тестов / 87 наборов (+8), пороги держатся, eslint 0
...
**Сделано (этап 87 — колонка «Качество» в админке всегда прочерк).**
...
- **`postprod-video-summary.ts` — тот же приём, что `session-summary.ts`
  (этап 51/87, админский список сессий), но проще.** JSON-путь читается
...
## Сделано (этап 142 — «Балансы»: остаток, который не в деньгах)

Из постановки п.36 (TODO §III) оставались открытыми двое провайдеров, у
которых остаток ЕСТЬ, но не в долларах: ElevenLabs считает символы,
SerpApi — поиски. Экран отвечал про обоих «остаток не в деньгах —
отдельная задача».

**Единицы не переводятся в доллары, и это главное решение этапа.**
Соблазн очевиден: одна колонка, всё складывается, одна сумма «сколько у
нас всего осталось». Но цена символа зависит от тарифа и меняется без
нашего участия. Пересчитанная нами сумма разошлась бы со счётом
провайдера — и заметить это было бы нечем, потому что число выглядело
бы правдоподобно. Экран показывает то, что провайдер сказал, в его же
единицах и с его же датой сброса: сто тысяч символов до завтра и до
конца месяца — разные новости.

**Перебор лимита показывается перебором.** У ElevenLabs остаток не
приходит готовым: приходят `character_count` (истрачено) и
`character_limit` (сколько дают), остаток — их разность. На тарифах с
`current_overage` она бывает отрицательной. Приводить такое к нулю
значило бы показать «осталось 0» там, где правда — «уже должны».

**Ключ SerpApi не попадает никуда, кроме самого запроса.** Заголовка их
API не принимает, ключ идёт строкой запроса, а `detail` уходит и на
экран, и в журнал. Поэтому при отказе пишется код ответа — без адреса и
без тела ответа (тело провайдеры нередко повторяют запрос целиком).
Отдельный тест следит, что ключа нет ни в одном поле строки.

Разбор обоих ответов — чистый `common/unit-balance.ts`, по той же
причине, по которой отдельно живёт `xai-balance.ts`: ключей в песочнице
нет, а форма чужого JSON — ровно то место, где ошибаются. Запрос,
таймаут и перевод отказов в причины — общая половина на двоих в
сервисе: различаются у них только адрес, способ передать ключ и разбор,
а написанное дважды расходится на первой же правке.

Новых переменных окружения не потребовалось: у обоих остаток
спрашивается тем же ключом, которым продукт к ним и ходит
(`VOICE_API_KEY`, `SERPAPI_API_KEY`), и отсутствие ключа — по-прежнему
«не настроено» с именем переменной, а не ошибка.

Мутациями проверено: приведение перебора к нулю краснеет, секунды,
принятые за миллисекунды, краснеют, ноль поисков, потерянный из-за
`||` вместо `??`, краснеет, адрес в сообщении об ошибке краснеет,
пропавший заголовок с ключом краснеет.

**Аудит этапа — три находки, все закрыты.**

1. **Трёх провайдеров спрашивали по очереди.** Таймаут у каждого свой
   — десять секунд, и худший случай складывался в тридцать: запрос
   переставал укладываться в таймаут функции. Ровно эта поломка у
   продукта уже была на сборке звуковых дорожек (аудит этапа 139), а
   уйти в фон у serverless нельзя. Теперь `Promise.all`: ни один из
   трёх наружу не бросает, так что терять там нечего. Тест открывает
   ворота только когда пришли все три запроса — при последовательном
   ходе первый ждал бы их вечно.

2. **Ключ мог уехать в сообщении сорвавшегося запроса.** Проверка «в
   `detail` нет ключа» была написана только для отказа с кодом, а
   ветка исключения отдавала текст ошибки как есть — и часть отказов
   `fetch` называет адрес прямо в тексте («Failed to parse URL from
   …»), а у SerpApi ключ стоит в адресе. `detail` уходит и на экран, и
   в журнал. Держать в голове список «какие сообщения содержат адрес»
   бессмысленно, поэтому ключ вычищается из любого текста перед
   выходом — вместе со своим URL-кодированным видом, потому что в
   адрес он попадает уже закодированным.

3. **Инвариант «список явный, чтобы забытый провайдер не молчал» не
   держался ничем.** Он записан в шапке файла с этапа 131 и был правдой
   ровно до первой правки: этот этап убрал двоих из списка «остаток не
   отдаёт» и добавил их отдельными читателями, и пропажа одного из них
   выглядела бы как обычная пустая строка. Теперь набор провайдеров
   экрана сверяется с `AI_PROVIDERS` — и на совпадение, и на
   отсутствие дублей.

## Сделано (этап 143 — порог остатка и сторож в канале ошибок)

Последний открытый кусок п.36 (TODO §III). Страница отвечает тому, кто
на неё зашёл, — а кончившийся баланс тем и опасен, что заходить на неё
в этот день никто не собирался.

**Крон `balances-watch`, раз в сутки в 5:30 UTC** — за полчаса до
суточного отчёта, чтобы новость о балансе пришла раньше сводки.
Именно раз в сутки: порог отвечает на вопрос «когда пополнять», а не
«всё ли ещё работает», — он на то и порог, чтобы оставался запас на
день-другой. Двенадцать сообщений об одном и том же числе в день
выключают канал вернее, чем отключённый крон: сторож остаётся
формально исправным и перестаёт работать.

**Порог свой у каждого провайдера** — `BALANCE_ALERT_USD`,
`BALANCE_ALERT_ELEVENLABS_CHARACTERS`, `BALANCE_ALERT_SERPAPI_SEARCHES`.
Общего быть не может: остаток у троих меряется в долларах, символах и
поисках, а привести одно к другому нечем — цена символа зависит от
тарифа (то же решение, что на этапе 142). Ноль означает «не
сторожить»: у провайдера с неровным расходом молчание лучше
еженедельного ложного крика. А вот мусор в переменной откатывается к
УМОЛЧАНИЮ, а не к нулю: опечатка, тихо выключившая сторожа, — худший
исход из возможных, потому что он есть, настроен и молчит.

**«Остаток не читается» — тоже повод написать.** Сторож, переставший
видеть, обязан сказать об этом сам: молчащая проверка неотличима от
проверки, у которой всё хорошо, и это худший исход — страница
показывает прочерк, а в канал не приходит ничего. «Не настроено» и
«провайдер остатка не отдаёт» поводом не считаются: это известные
состояния, они не меняются сами, и ежедневный крик о них научит не
читать канал.

**Сообщение на каждый повод отдельно**, с отпечатком
`balance-<повод>:<провайдер>` — без переменных частей, иначе
дедупликация `TelegramNotifyService` не срабатывает вовсе (сумма в
отпечатке означала бы новое сообщение на каждый доллар). Общим
сообщением молчание про один провайдер прятало бы крик про другого.
Сторож смотрит свежие числа, а не пятиминутный кеш: кеш заведён ради
чужих ограничений частоты при живом человеке у экрана, и отвечать
сторожу пятиминутной стариной значило бы сторожить не то, что есть.

Счётчик «отправлено» в истории крона считает ОТПРАВЛЕННЫЕ, а не поводы:
канал может быть не настроен, и `alert` честно отвечает `false` —
записать в историю «отправлено 2» было бы неправдой.

Мутациями проверено: выключение сторожа мусором в переменной краснеет,
крик ровно по порогу краснеет, молчание о нечитаемом остатке краснеет,
сравнение порога с непрочитанным остатком краснеет, кеш вместо свежих
чисел краснеет, отпечаток с переменной частью краснеет.

**Аудит этапа — три находки, все закрыты.**

1. **Сообщения уходили по очереди.** Ровно та поломка, которую этот же
   аудит закрыл этапом раньше, и написанная сразу после него: у
   отправки в Telegram свой таймаут в пять секунд, три подряд ложатся
   поверх десяти секунд на сами остатки — двадцать пять в худшем
   случае, снова мимо таймаута функции. Теперь `Promise.all`: отпечатки
   у поводов разные, записи дедупликации друг с другом не спорят.

2. **Ноль выключал сторожа только наполовину.** Он гасил сравнение с
   порогом, но оставлял крик «остаток не читается», и выключить тот
   было нечем. Живой случай на это прямо напрашивается: у аккаунта xAI
   на постоплате предоплаченного остатка нет ВООБЩЕ, и код сам так и
   пишет — сторож слал бы про это сообщение каждый день, вечно, про
   состояние, которое не чинится. Канал, кричащий про нечинимое,
   перестают читать целиком, вместе с тем, что чинится. Теперь ноль
   означает «не сторожить этого провайдера», оба повода сразу. Заодно
   порог стал выбираться по ВИДУ остатка: кредиты провайдера без
   своего порога больше не сравниваются с долларовым — число
   получалось, а смысла в нём не было.

3. **Выключенный сторож выглядел в точности как здоровый.** В историю
   крона писалось `checked: 10` — число СТРОК, а не проверок: семерых
   не спрашивают вовсе, а выключенных нулём не сторожат. И «ноль
   поводов» у здорового и у выключенного читалось одинаково. Теперь
   пишется `watched` — скольких сторожили на самом деле. Он же закрыл
   вторую половину той же дыры: три новых порога появились на экране
   «Настройки», где ноль назван выключением, а не порогом «ноль
   долларов». До этого «сторож выключен» нельзя было увидеть нигде, не
   зная заранее, что это надо искать.

Пункт 36 закрыт полностью.
