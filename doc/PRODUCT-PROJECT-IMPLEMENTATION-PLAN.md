...
## Итоговая сверка (обновлена после аудита кода трёх последних ТЗ, см. ниже)

- Backend: **185 маршрутов в 39 контроллерах** (этап 78 добавил два:
  `GET /admin/workflow-funnel` и `GET /admin/workflow-funnel/cohort-conversion`
  в `AdminPanelController`; ни один из пунктов этапа не убирал
  маршрутов), eslint 0 ошибок по всем файлам, изменённым этапом
  (`--max-warnings 0`), `prisma validate`, все **42** миграции подряд на чистом Postgres 16
  (**38** таблиц — этап 78 добавил ровно одну новую
  таблицу, `workflow_stage_events` (append-only журнал переходов по
  трём воркфлоу: сессия/пакетная генерация/A-B-варианты), и ровно одну
  миграцию, `20261020090000_workflow_stage_events`, применённую и
  подтверждённую на локальном Postgres 16).
  **2051 теста / 145 наборов** (2044/144 после этапа 78 + 7 тестов в
  новом `admin-panel.controller.spec.ts`, добавленном аудитом кода трёх
  последних ТЗ — см. «Внеплановый фикс (аудит трёх последних ТЗ)» ниже)
  ...
  оставшемся списке не появляется. Прирост с этапа 77 (2001/143) до
