...
Статус: **v2 — открытые вопросы закрыты, аудит проведён** · 17.09.2026
...
9. **Одобрение промпта.** Применение или откат скетча сбрасывает одобрение промпта (`generationPrompt.approvedAt`), если слот сессии входит в план референсов и меняется текст сопоставления референсов (`referenceMappingText`) или выбор между первым кадром и референсом (§5.6). Приём тот же, что `resetApproval` в `ProjectSessionService.updateSnapshot`.
...
| `POST /sketches/delete-original` | `{ target }` → `{ slot, updatedRefs: number }` | Удалить оригинал (§4, п. 10). POST, а не DELETE с телом _(аудит А-17)_: не все прокси пропускают тело у DELETE |
...
- 422 — отказ модели;
- 429 — квота исчерпана; в теле `quota` и `upgrade: PlanId | null`;
- 502 — сбой провайдера.
...
- Превью персонажа (§5.4) расходует ту же интерактивную квоту.
...
6. «Вернуть оригинал» → в сводке «оригинал», следующий ролик с оригиналом; одобрение промпта сброшено, если менялся план.
...
**Фаза 2 — бренд, проекты, пакеты (5–6 дней)**
