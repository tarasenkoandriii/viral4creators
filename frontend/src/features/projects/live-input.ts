...
/**
 * Перехватывать ли нажатие, когда ввод идёт в кадр.
 *
 * Перехватывается всё, кроме `Escape` — он и есть выход из кадра
 * (снимает фокус), иначе человек оказался бы заперт.
 *
 * `Tab` сначала тоже был исключён — по тому же доводу «надо чем-то
 * выйти». Это была ошибка: на форме входа Tab — ОСНОВНОЙ способ
 * перейти от логина к паролю, то есть исключение выбивало его ровно
 * там, где он нужен постоянно. Одного `Escape` для выхода достаточно.
 */
...
/**
 * На сколько пикселей прокручивать чужую страницу одним нажатием
 * экранной кнопки.
 *
 * Кнопки появились не от хорошей жизни: колесо в webview Telegram до
 * страницы не доходит вовсе (проверено счётчиками реле — `wheel=0` при
 * живой мыши и клавиатуре), а на телефоне колеса нет и в помине.
 * Кнопка не зависит ни от того, ни от другого.
 *
 * Доля экрана, а не фиксированное число: на телефоне и на десктопе
 * «один экран вниз» означает разное. Нижний предел — на случай, когда
 * кадр ещё не измерен (первый кадр не пришёл) и высота нулевая: без
 * него нажатие не делало бы ничего.
 */
export function scrollStepPixels(viewportHeight: number): number {
  const step = Math.round(viewportHeight * 0.8);
  return step < 120 ? 120 : step;
}
