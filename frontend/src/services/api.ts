...
export async function uploadProductImage(
  ...
  // Подтверждение — отдельный шаг (В-1.8, этап 123). До него сервер про
  // это фото ничего не записывает: ссылку он выдал, но выдать ссылку и
  // загрузить файл — разные события, и раньше сорвавшийся PUT оставлял
  // сессию с путём, по которому ничего нет. Мастер при восстановлении
  // показывал фото загруженным, а генерация упиралась в техническую
  // ошибку скачивания.
  await api.post(`/sessions/${sessionId}/product/image/confirm`, {
